...
var initCons = {
    run: function (rooms) {
        for (let i in Game.spawns) {
            ...
            if (amount >= 10 && spawnInfo.room.find(FIND_MY_CREEPS, {
            ...
            } else if (amount >= 13 && spawnInfo.room.find(FIND_MY_CREEPS,
                {
                filter: function (creep) {
                ...
                if (randCase < 50) {
                    ...
                    sucSpa = spawnInfo.spawnCreep([RANGED_ATTACK, RANGED_ATTACK, TOUGH, TOUGH, MOVE, MOVE, MOVE],
                        nameBefore + spawnInfo.memory.nextCreep,
                        {memory: {'belongs': 0, 'role': 'weapon'}});
                }
            } else {
                if (amount <= 10) {
                    ...
                    sucSpa = spawnInfo.spawnCreep([WORK, WORK, WORK, CARRY, MOVE, MOVE],
                        nameBefore + spawnInfo.memory.nextCreep);
                }
            }
